<div class="modal animate-ModalSlide {{ $errors->any() ? 'block' : '' }}" id="modalscrollable">
    <div class="relative w-auto pointer-events-none sm:max-w-lg sm:my-7 sm:mx-auto z-[99] h-96">
        <div class="relative flex flex-col w-full pointer-events-auto bg-white dark:bg-slate-800 bg-clip-padding rounded overflow-hidden max-h-full">
            ...
            @php
                $oldInputs = old('id_agama', []);
            @endphp

            <!-- Scrollable Form Content -->
            <div class="relative flex-auto p-4 text-slate-600 dark:text-gray-300 leading-relaxed overflow-y-auto">
                <form action="{{ route('agama.store') }}" method="POST" id="form-agama">
                    @csrf
                    @if (count($oldInputs) > 0)
                        @foreach ($oldInputs as $i => $id_agama)
                            <div class="agama-item grid md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 gap-2 mt-2">
                                {{-- Kode agama --}}
                                @if ($i > 0)
                                ...
                                @endif
                                    <div class="relative z-0 w-full group">
                                        <label class="font-medium text-sm text-slate-600 dark:text-slate-400">ID Agama</label>
                                        <input type="text" name="id_agama[]" value="{{ old("id_agama.$i") }}" placeholder="AG001"
                                            class="form-input w-full rounded-md mt-1 border dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 {{ $errors->has("id_agama.$i") ? 'border-red-500 is-invalid' : '' }}">
                                        @error("id_agama.$i")
                                ...
                                <div class="sm:col-span-6 md:col-span-6 lg:col-span-6 xl:col-span-6">
                                    <div class="relative z-0 w-full group">
                                        <label class="font-medium text-sm text-slate-600 dark:text-slate-400">Agama</label>
                                        <input type="text" name="agama[]" value="{{ old("agama.$i") }}" placeholder="Islam"
                                            class="form-input w-full rounded-md mt-1 border dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 {{ $errors->has("agama.$i") ? 'border-red-500 is-invalid' : '' }}">
                                        @error("agama.$i")
                                            <div class="invalid-feedback text-red-500 font-medium text-sm">
                                                {{ $message }}
                                            </div>
                    ...
                    @else
                        ...
                        <div class="agama-item grid md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 gap-2 mt-2">
                            <div class="sm:col-span-6 md:col-span-6 lg:col-span-6 xl:col-span-6">
                                <div class="relative z-0 w-full group">
                                    <label class="font-medium text-sm text-slate-600 dark:text-slate-400">ID Agama</label>
                                    <input placeholder="AG001" type="text" name="id_agama[]" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none">
                                </div>
                            </div>
                            <div class="sm:col-span-6 md:col-span-6 lg:col-span-6 xl:col-span-6">
                                <div class="relative z-0 w-full group">
                                    <label class="font-medium text-sm text-slate-600 dark:text-slate-400">Agama</label>
                                    <input placeholder="Islam" type="text" name="agama[]" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none">
                                </div>
                            </div>
                        </div>
                    @endif
            ...
            <div class="flex flex-wrap shrink-0 justify-end p-3 border-t border-dashed dark:border-gray-700">
                <a href="#" class="addAg inline-block focus:outline-none text-primary-500 hover:bg-primary-500 hover:text-white bg-transparent border border-gray-200 dark:bg-transparent dark:text-primary-500 dark:hover:text-white dark:border-gray-700 dark:hover:bg-primary-500 text-sm font-medium py-1 px-3 mr-2 rounded">
                    Add Form
                </a>
